...
export function buildSeedReports(): Record<string, Report[]> {
  ...
  return {
    gaza: [
      ...
      seed('gaza', 'Medical',    'emergency', 6,  'Burns from fire, need dressing changes',        'Jabalia',        160, g, 'assigned'),
      seed('gaza', 'Shelter',    'today',     14, 'School shelter overcrowded',                    'Al-Rimal',       190, g, 'assigned'),
      seed('gaza', 'Water',      'safe',      4,  'Water access restored',                         'Sheikh Radwan',  220, g, 'resolved'),
      seed('gaza', 'Food',       'safe',      11, 'Aid convoy arrived',                            'Deir al-Balah',  300, g, 'resolved'),
    ],
    khartoum: [
      ...
      seed('khartoum', 'Food',       'today',     50, 'Community kitchen supplies running low',      'Al-Thawra',       150, k, 'assigned'),
      seed('khartoum', 'Shelter',    'safe',      22, 'Tent allocation complete',                    'Soba',            240, k, 'resolved'),
    ],
    istanbul: [
      ...
      seed('istanbul', 'Food',    'safe',  40, 'Community canteen set up by municipality',       'Bağcılar',      300, i, 'resolved'),
    ],
    karachi: [
      ...
      seed('karachi', 'Medical',    'emergency', 3,'Child with fever after flood exposure',         'Orangi',   70, p, 'assigned'),
      seed('karachi', 'Shelter',    'safe',     15,'Temporary shelter allocated at school',        'Baldia',   180, p, 'resolved'),
    ],
    cairo: [
      seed('cairo', 'Water',   'safe',     4,  'Seasonal well drying up earlier than expected',  'Rod al-Farag', 40,  c),
      seed('cairo', 'Shelter', 'safe',     8,  'Displaced by Nile flooding, temporary housing',  'Imbaba',       200, c, 'resolved'),
    ],
  }
}
